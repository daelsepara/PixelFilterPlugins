...
namespace xbrz
{
	...
	*/
	...
	inline void fillBlock(uint32_t* trg, int pitch, uint32_t col, int n) { fillBlock(trg, pitch, col, n, n); }
	...
	{
		if (srcPitch < srcWidth * static_cast<int>(sizeof(uint32_t)) || trgPitch < trgWidth * static_cast<int>(sizeof(uint32_t)))
		{
			return;
		}
	...
	inline void nearestNeighborScale(const uint32_t* src, int srcWidth, int srcHeight, uint32_t* trg, int trgWidth, int trgHeight)
	{
	...
	template <uint32_t N> inline unsigned char getByte(uint32_t val) { return static_cast<unsigned char>((val >> (8 * N)) & 0xff); }
	...
	template <unsigned int M, unsigned int N> inline uint32_t gradientRGB(uint32_t pixFront, uint32_t pixBack) //blend front color with opacity M / N over opaque background: http://en.wikipedia.org/wiki/Alpha_compositing#Alpha_blending
	{
	...
	template <unsigned int M, unsigned int N> inline uint32_t gradientARGB(uint32_t pixFront, uint32_t pixBack) //find intermediate color between two colors with alpha channels (=> NO alpha blending!!!)
	{
...
#define FORCE_INLINE inline
	...
	inline double distRGB(uint32_t pix1, uint32_t pix2)
	{
	...
	inline double distYCbCr(uint32_t pix1, uint32_t pix2, double lumaWeight)
	{
	...
	-----------------
	*/
	template <class ColorDistance> FORCE_INLINE //detect blend direction
...
#undef DEF_GETTER
	...
	-------------
	*/
	template <class Scaler, class ColorDistance, RotationDegree rotDeg> FORCE_INLINE //perf: quite worth it!
...
#undef i
	...
	template <class ColorGradient> struct Scaler2x : public ColorGradient
	{
		...
		template <class OutputMatrix> static void blendLineShallow(uint32_t col, OutputMatrix& out)
		{
			alphaGrad<1, 4>(out.template ref<scale - 1, 0>(), col);
			alphaGrad<3, 4>(out.template ref<scale - 1, 1>(), col);
		}

		template <class OutputMatrix> static void blendLineSteep(uint32_t col, OutputMatrix& out)
		{
			alphaGrad<1, 4>(out.template ref<0, scale - 1>(), col);
			alphaGrad<3, 4>(out.template ref<1, scale - 1>(), col);
		}

		template <class OutputMatrix> static void blendLineSteepAndShallow(uint32_t col, OutputMatrix& out)
		{
		...
		template <class OutputMatrix> static void blendLineDiagonal(uint32_t col, OutputMatrix& out)
		{
			alphaGrad<1, 2>(out.template ref<1, 1>(), col);
		}

		template <class OutputMatrix> static void blendCorner(uint32_t col, OutputMatrix& out)
		{
	...
	void scale(size_t factor, const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight, ColorFormat colFmt, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
	{
		switch (colFmt)
		{
	...
	void Copy(uint32_t* dst, unsigned char* src, int srcx, int srcy)
	{
		auto Channels = 3;
		...
		for (auto y = 0; y < srcy; y++)
		{
			...
			{
				auto index = y * srcx + x;
				auto pixel = index * Channels;
				auto r = src[pixel];
				auto g = src[pixel + 1];
				auto b = src[pixel + 2];

				dst[index] = (255 << 24) + (r << 16) + (g << 8) + b;
			}
		}
	}

	void Copy(unsigned char* dst, uint32_t* src, int srcx, int srcy)
	{
		auto Channels = 3;
		...
		for (auto y = 0; y < srcy; y++)
		{
			for (auto x = 0; x < srcx; x++)
			{
				...
				dst[pixel] = (unsigned char)(src[index] >> 16);
				dst[pixel + 1] = (unsigned char)((src[index] & 0x00ff00) >> 8);
				dst[pixel + 2] = (unsigned char)(src[index] & 0x0000ff);
			}
		}
	}
}
